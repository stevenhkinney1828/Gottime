...
- 🔄 **New finding, one step later:** **Export & upload to TestFlight** now fails with `Cloud signing permission error` — cloud-managed distribution signing specifically requires an **Admin**-role API key; the "App Manager" role recommended earlier in this doc turns out to be insufficient for this one capability (though correct for everything else it's used for). SETUP.md corrected to recommend Admin outright. **Waiting on the owner to generate a new Team API key with Admin access** and update two of the four secrets (`APP_STORE_CONNECT_API_KEY_ID`, `APP_STORE_CONNECT_API_KEY_P8` — Issuer ID and Team ID don't change). See DECISIONS.md for the full sequence of three findings.
