...
- 🔄 Owner re-saved `APP_STORE_CONNECT_API_KEY_P8` with the corrected content (confirmed it now includes the `-----BEGIN PRIVATE KEY-----`/`-----END PRIVATE KEY-----` lines, likely what was missing the first time). Triggering a fresh run now to confirm. Two physical iPhones still needed too, for the actual two-way call test once a signed build reaches TestFlight.
