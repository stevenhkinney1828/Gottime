...
**Current phase: Phase 4 — Voice proof (all code written — backend, iOS, and CI signing pipeline; 2-part owner gate remains before any of it can be tested for real — see bottom)**
...
- ✅ **`ios-ci.yml`'s `sign-and-upload` job fully implemented** (archive → export → upload to TestFlight, driven entirely by an App Store Connect API key — no local Mac or Keychain ever needed). **Written, carefully checked against Apple's own documentation, and passed `actionlint`/a real YAML-parser check — but genuinely not yet verified by an actual run**, unlike everything else in this document marked ✅: there is no fake-credentialed way to test `xcodebuild` signing at all, so the owner's real API key (see the gate just below) is the only thing that can actually prove this pipeline works. Expect this to need at least one round of real iteration the first time it runs. See DECISIONS.md for exactly what was and wasn't possible to verify ahead of time. A real `ios-ci.yml` run did confirm the job's structure is sound within GitHub's actual workflow engine (not just this session's own parser check): its new steps all correctly no-op'd via the same `ready`-guard pattern the stub already used, and the always-running Simulator job stayed green alongside it.
- 🚧 Still needed before a real two-way call can be tested, or before the pipeline above gets its first real run: two physical iPhones, and an App Store Connect API key + Internal Testing group. See SETUP.md for the click-by-click walkthrough.
